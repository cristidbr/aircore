...
function minifyResources( resources )
{
    ...
    var shell = require( "child_process" ), compress = "7za a -tgzip ", compress_cmd = [];
    ...
    for( var i in html ) {
        ...
        compress_cmd.push( compress + ( compressPath + fname + '.gz' ) + " " + ( minifyPath + fname ) );
    ...
    for( var i in css ) {
        if( generate_compiled.indexOf( css[ i ] ) >= 0 ) {
            ...
            file = fs.readFileSync( sourcePath + css[ i ], 'utf8' );
            minified = new minifyCSS().minify( file ).styles;
            fs.writeFileSync( minifyPath + fname, minified, 'ascii' );

            compress_cmd.push( compress + ( compressPath + "css_" + css[ i ].replace( "css/", "" ) + '.gz' ) + " " + ( minifyPath + fname ) );
    ...
    for( var i in js ) {
        if( generate_compiled.indexOf( js[ i ] ) >= 0 ) {
            ...
            compress_cmd.push( compress + ( compressPath + "js_" + js[ i ].replace( "js/", "" ) + '.gz' ) + " " + ( minifyPath + fname ) );
